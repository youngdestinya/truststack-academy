...
import { skillsForTrack } from './certificate-skills';
export { authorized } from './admin-auth';
export function makeCert({student,track,date_issued}) {
 ...
 const cert={id:`${new Date().getUTCFullYear()}-${hex}`,student:student.trim(),track:track.trim(),date_issued:date,hash_prefix:hex,status:'verified',signed_by:'Destiny Young',signer_title:'Founder and Chief Technology Architect, Truststack Academy and Educational Services Ltd',signed_for:'Truststack Academy'};
 cert.core_skills=skillsForTrack(cert.track);
 ...
 cert.verify_url=`${process.env.NEXT_PUBLIC_BASE_URL || 'https://truststack.academy'}/verify?id=${cert.id}`;
 return cert;
}
