...
function signature(payload) {
  const secret = process.env.ADMIN_SESSION_SECRET || process.env.ADMIN_KEY || process.env.ADMIN_PASSWORD;
  if (!secret) return '';
...
export function adminCredentialsMatch(email, password) {
  ...
  const expectedPassword = process.env.ADMIN_PASSWORD || process.env.ADMIN_KEY;
  return safeEqual(expectedEmail, String(email || '').trim().toLowerCase()) && safeEqual(expectedPassword, password);
}
