(function () {
  ...
  function addSharedStyles() {
    ...
    style.textContent = `
      ...
      *, *::before, *::after, html, body, button, input, select, textarea, h1, h2, h3, h4, h5, h6, p, span, a, strong, small, label, div, nav, header, footer {
      ...
      .ts-trust-strip{background:#00c7e8!important;border-color:rgba(6,22,45,.14)!important;color:#06162d!important;min-height:68px!important;height:auto!important;padding-top:12px!important;padding-bottom:12px!important}
      .ts-trust-strip *{color:#06162d!important;opacity:1!important}
      .ts-social-strip{display:flex!important;align-items:center;gap:.65rem!important}
      .ts-social-strip a{display:grid;place-items:center;width:32px;height:32px;border-radius:999px;color:#fff!important;box-shadow:0 4px 12px rgba(6,22,45,.16);transition:transform .18s ease,box-shadow .18s ease}
      .ts-social-strip a:hover{transform:translateY(-2px) scale(1.05);box-shadow:0 7px 16px rgba(6,22,45,.24)}
      .ts-social-strip svg{width:16px;height:16px;color:#fff!important;fill:#fff!important}
      ...
      /* Hide hero canvas constellation particles */
      canvas, section canvas { display: none !important; }
  ...
  function brandHomeLinks() {
    colorBrandAcademy();
    // ensureBrandInHeader removed to prevent navbar duplicate text
  ...
  function refineBuilderProfile() {
    ...
    if (!card || card.classList.contains('ts-builder-profile') || card.classList.contains('text-center')) return;
  ...
  document.addEventListener('click', (event) => {
    ...
    if (target === '/home.html#whytruststack' && isHome && focusWhyTrustStack(true)) {
