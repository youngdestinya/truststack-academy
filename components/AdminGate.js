...
export default function AdminGate({ children, title = 'Protected workspace' }) {
  ...
  const login = async () => {
    const response = await fetch('/api/admin/session', { method: 'POST', headers: { 'x-admin-key': key } });
    if (response.ok) { setAllowed(true); setError(''); } else setError('Invalid admin key.');
  };
